...
var stdioMirroredTools = map[string]bool{
	...
	"get_program_agent_jobs":            true,
	"rebind_program_lane":               true,
	"create_session":                    true,
...
func TestCatalogImplementedAllHaveStdioMirror(t *testing.T) {
	...
	if len(stdioMirroredTools) != 82 {
		t.Errorf("stdioMirroredTools size=%d want 82 (sync with stdioToolNames.ts)", len(stdioMirroredTools))
	}
}
...
func TestCapabilityForSamples(t *testing.T) {
	cases := []struct {
		...
		{"get_hermes_insights", "/api/v1/hermes/insights", "agent"},
	}
