...
export function buildGitDirtyRemediatePrompt(bridge: AgentBridgeResponse | undefined): string {
  ...
  return [
    'Remediate Engineer git dirty via Git Bridge — propose commit (default) or stash if operator prefers.',
    'This is NOT a magic auto-clean. Operator approval is required before git_commit or git_stash.',
    '',
    ...
    '3. Or if operator asks to stash: request_operator_approval → git_stash (never drop)',
