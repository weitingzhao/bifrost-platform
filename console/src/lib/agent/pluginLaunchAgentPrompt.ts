...
function marketDataPrompt(ctx: PluginLaunchPromptContext): string {
  ...
  return [
    ...
    `   ${apply}`,
    '   Ensure bifrost-market-data:0.2.0 is present on cluster nodes before apply (import if needed).',
    '4. Verify — run:',
    `   ${verify}`,
    ...
    '',
    '## Must-not',
    '- Do not apply k8s/base when seat is stg or prod (wrong namespace).',
