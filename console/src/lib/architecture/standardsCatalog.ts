...
export const ACTUATION_API_ROUTES: ActuationApiRoute[] = [
  ...
  { phase: 'P1', method: 'GET', route: '/api/v1/cluster/nodes/{name}/power', role: 'viewer', purpose: 'Compute node power + workloads' },
  { phase: 'P1', method: 'POST', route: '/api/v1/cluster/nodes/{name}/wake', role: 'operator', purpose: 'Wake-on-LAN compute node' },
  { phase: 'P1', method: 'POST', route: '/api/v1/cluster/nodes/{name}/poweroff', role: 'admin', purpose: 'Drain + power off compute node' },
  { phase: 'P1', method: 'POST', route: '/api/v1/cluster/addons/metrics-server/ensure', role: 'admin', purpose: 'Install metrics-server (Layer A)' },
  { phase: 'P3', method: 'POST', route: '/api/v1/cluster/addons/kube-prometheus-stack/ensure', role: 'admin', purpose: 'Install kube-prometheus-stack (Layer B)' },
  { phase: 'P2', method: 'GET', route: '/api/v1/cluster/join-profiles', role: 'viewer', purpose: 'K3s join profile list (L0)' },
...
export const OBSERVABILITY_LAYERS: LayerDescription[] = [
  {
    ...
    notes: 'Available from P1; requires metrics-server install · Rocket → Cluster · Satellite → Bus Status summary',
  },
  {
    layer: 'B — Probe',
