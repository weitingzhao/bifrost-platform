...
export const DAILY_OPS_FLEET_DESK = {
  ...
  rules: [
    ...
    'Remediate primary CTA follows highest-priority Checklist×Fleet blocker (fail>degraded; manual/observe before AI-fixable at same severity): full_auto/semi_auto+scope → Agent Fix / AI Fix · Operator Plan; manual/observe/null-scope → Manual next step (no sparkles AI Fix); mixed → primary manual + outline Also: AI Fix (sibling).',
    'Engineer CRITICAL: fleet cell Agent Fix stays disabled; inline Operator Plan when AI-fixable, else Manual next (e.g. Mac seat). Full Operator Plane page is escape hatch only.',
  ...
  acceptance: [
    ...
    'Q8: Engineer CRITICAL → blocker-typed CTA (manual-next for seat/manual/observe; AI Fix · Operator Plan when AI-fixable); Full page → secondary; canOperate gates; D10 copy on remediate.',
