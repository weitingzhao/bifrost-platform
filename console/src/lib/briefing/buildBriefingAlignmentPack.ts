...
export function buildBriefingAlignmentPack(input: BriefingSnapshotInput): string {
  ...
  return [
    ...
    '8. `console/src/lib/architecture/standardsCatalog.ts` + `api/internal/server/server.go` — API contract (UI + code)',
    '9. `console/src/lib/architecture/blueprintCatalog.ts` + `agentProtocolCatalog.ts` — view matrix + Agent workflow',
