...
export const PAGE_COMPOSITION: PageCompositionAct[] = [
  {
    ...
    examples: [
      ...
      'Satellite Health: Probes | Runtime segments — OpsVerdictStrip PROBE RESULTS / SATELLITE RUNTIME · {ENV} (freshness, not system readiness badge)',
      'Rocket Health: Probes-first (self-health API/Console/Argo) — OpsVerdictStrip ROCKET HEALTH · {ENV}; Runtime deferred to Cluster Layer B / Observability',
      'Network: OpsVerdictStrip NETWORK · GROUND — live probe lamp; Health panel is Body evidence',
      'K3s node lifecycle (Wake/Join/cordon/drain): Rocket → Cluster (Ground Compute page removed)',
      ...
      'Plugin Gallery: OpsVerdictStrip PLUGIN BUS rollup + registry cards with Open IB Gateway / Open Market Data; detail pages own probes (Gallery ≠ Publish)',
      'IB Gateway: OpsVerdictStrip + Live status & cutover OpsSection + Reconnect (manage ≠ Launch Plugin publish)',
      ...
      'Launch Agent: LaneStateStrip + AI Launch Agent (+ Operator Plane evidence) + LaneOperateSplit — Detect→Approve→Deploy→Verify→Live; executor deploy_mac_mini.sh (not Tekton / not in-cluster); manual Update = escape',
    ],
  },
  {
    act: 'Body',
    ...
    examples: [
      ...
      'Deploy Satellite: same agent-driven polish as Launch Rocket — AI Deploy + checklist GO gate; Step detail observe-only (phase-click Log Tail, line numbers, last-line freshness); Supporting evidence = supply chain + gate history; Toolbox = Advanced recovery (GitOps) + workflow',
      'Launch Plugin: LaneOperateSplit shell parity — left: target/seat + PluginStepCommandCenter (Detect→Live, make/kubectl); right: light Launch checklist (not Tekton GO gate) + Acceptance + Supporting evidence (Gallery/IB/MD) + Toolbox; Record buttons = evidence supplement',
      'Launch Agent: LaneOperateSplit shell parity — left: primary/standby Segment + PluginStepCommandCenter (Detect→Deploy→Live); right: light checklist + L-1 boundary + Runner pulse + Toolbox (manual Update escape)',
      'Cluster: Verdict meta chips scroll to Body anchors (#cluster-issues = merged triage+fleet, #placement-violations under Facility → Policy matrix, …)',
      'Cluster: OpsSection Node operations wizard + nodes table (+ NS/workloads)',
      ...
      'Plugin Gallery: registry directory + bus rollup only — Open IB Gateway / Open Market Data for detail; Need publish? → Launch Desk → Plugin',
      'IB Gateway: Live + Cutover nested flat under one OpsSection; Reconnect on verdict strip',
    ],
  },
  {
    act: 'Actions',
    ...
    rules: [
      ...
      'Rocket / Satellite / Plugin / Agent lane operate pages: LaneStateStrip is the Verdict equivalent — page-level Agent CTA must stay on the strip (AI Release / AI Deploy / AI Launch Plugin / AI Launch Agent), not inside Advanced recovery',
    ...
    examples: [
      ...
      'Launch Plugin: AI Launch Plugin on LaneStateStrip (+ Evidence links); Detect→Live in primary pane; Record = evidence only; light checklist NO-GO → Agent Fix (runtime repair ≠ publish)',
      ...
      'Launch Agent: AI Launch Agent on LaneStateStrip; Detect→Deploy→Live in primary pane; Record = evidence only; manual Update in Toolbox = escape; Dock host/deploy CTA → Launch Agent; Operator Plane → heartbeats/MCP',
...
export const MANDATORY_MAPPING: MandatoryMappingRow[] = [
  ...
  {
    interaction: 'Page verdict (Mission Control / Rocket Cluster / Plugin)',
  ...
  },
  {
    ...
    use: 'SeatStrip + PartnerStrip via ShellNavSidebar seatContent/partnerContent; Mission groups (Satellite / Rocket / Plugin) defaultOpen; Trade omits slots (zero-change)',
