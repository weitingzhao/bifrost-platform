  ...
  // =========================================================================
  {
    ...
    items: [
      ...
      },
      {
        ...
        label: 'IB data feed',
        group: 'feed',
        idPattern: '(^|[-_])ib($|[-_])|ib-gateway|ibkr',
        healthyCriteria:
          'Matrix targets for IB reachability=ok. D10 BLOCKED: observe status only, no auto-fix.',
        fixScope: null,
        fixCapability: 'observe',
        manualAction:
          'IB Gateway runs on Mac Mini under TWS GUI. Requires physical/VNC: restart TWS, verify client_id, check API port. D10: no live trade execution.',
        boardProjection: {
          standardId: 'ib-feed',
          label: 'IB data feed',
          cell: { role: 'vendor', env: 'span' },
          group: 'feed',
          required: false,
          reason: 'Checklist projection · no dedicated IB matrix probe (D10 observe)',
        },
      },
      {
        id: 'hermes-tooling',
...
export const DAILY_OPS_CHECKLIST_META = {
  ...
  designDecisions: [
    ...
    'IB feed: observe-only (D10 BLOCKED); checklist confirms status, no AI fix path.',
    ...
    'Vendor feeds: Massive/Polygon uses stable massive-polygon probe id (not combined massive-ib); IB stays observe projection.',
