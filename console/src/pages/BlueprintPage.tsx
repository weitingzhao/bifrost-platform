...
type ConstitutionLens = 'core' | 'ai'

const LAYER_TO_ZONE: Record<string, BlueprintZoneId> = {
  Constitution: 'constitution',
  Spine: 'spine',
  Projection: 'projection',
}

const ZONE_HINT: Record<BlueprintZoneId, string> = {
  constitution: 'Principles · Owner exceptions · Auth · AI Native',
  spine: 'Milestone semantics · Live snapshot',
  projection: 'MCP tools · Actuation progress',
}

const BLUEPRINT_SECTIONS: Array<GovernanceCatalogSection<BlueprintZoneId>> = GOVERNANCE_LAYERS.flatMap(
  layer => {
    const id = LAYER_TO_ZONE[layer.layer]
    if (id == null) return []
    return [
      {
        id,
        label: layer.layer,
        badge: layer.changeRate.split('(')[0]?.trim() ?? layer.changeRate,
        summary: layer.content,
        hint: ZONE_HINT[id],
      },
    ]
  },
...
const BLUEPRINT_SHORTCUTS: Array<GovernanceCatalogShortcut<BlueprintZoneId>> = BOUNDARY_RULES.flatMap(
  r => {
    const sectionId = LAYER_TO_ZONE[r.answerLayer]
    if (sectionId == null) return []
    return [{ label: `${r.question} → ${r.answerLayer}`, sectionId }]
  },
)

export function BlueprintPage({ context }: { context?: OpsContextResponse }) {
  const [copyState, setCopyState] = useState<CopyState>('idle')
  // Exclusive section tabs — Constitution first; Spine / Projection on demand.
  const [zone, setZone] = useState<BlueprintZoneId>('constitution')
  const [constitutionLens, setConstitutionLens] = useState<ConstitutionLens>('core')
  ...
  return (
    <GovernanceCatalogShell
      description={
        <>
          HOW we build it — skim the three section summaries, then open one tab. Source:{' '}
      ...
      value={zone}
      onChange={setZone}
      shortcuts={BLUEPRINT_SHORTCUTS}
      tabAriaLabel="Blueprint section"
      onCopyForLlm={handleCopyForLlm}
      copyState={copyState}
    >
      {zone === 'constitution' ? (
        <section
          id={BLUEPRINT_ZONE_ANCHORS.constitution}
          className="flex scroll-mt-20 flex-col gap-3"
        >
          <SegmentControl
            ariaLabel="Constitution topic"
            className="self-start"
            options={[
              { value: 'core', label: 'Core · principles & control plane' },
              { value: 'ai', label: 'AI Native · mission & phases' },
            ]}
            value={constitutionLens}
            onChange={v => setConstitutionLens(v as ConstitutionLens)}
          />

          {constitutionLens === 'core' ? (
            <>
              ...
              <CatalogSection title="Design principles">
                <ul className="m-0 flex flex-col gap-2 py-2 pl-4 text-[var(--text-dense)]">
              ...
              <CatalogSection title="Actuation phases (definitions P0–P5)">
                <DenseDataTable>
                  <DenseTableHeader>
                    <DenseTableHeadRow>
                ...
                <p className="m-0 px-3 py-2 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
                  Phase labels define intent (Constitution). Implemented/total counts come from MCP tool
              ...
              <CatalogSection title="AI Platform capabilities">
                {AI_PLATFORM_CAPABILITIES.map(cap => (
                  <div key={cap.name} className="border-b border-[var(--border)] last:border-b-0">
                    ...
                    <p className="m-0 px-3 py-1 text-[var(--text-dense)] text-[var(--muted-foreground)]">
                      {cap.description}
                    </p>
      ...
      {zone === 'spine' ? (
        <section id={BLUEPRINT_ZONE_ANCHORS.spine} className="flex scroll-mt-20 flex-col gap-3">
          ...
          {context != null && (
            <CatalogSection title="Spine snapshot (live)">
              <div className="flex flex-col gap-1 px-3 py-3 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
      ...
      {zone === 'projection' ? (
        <section id={BLUEPRINT_ZONE_ANCHORS.projection} className="flex scroll-mt-20 flex-col gap-3">
