...
import {
  ...
  CONTROL_ROOM_PHASE4_DELIVERY_ITEMS,
  CONTROL_ROOM_PHASE4_VERSION,
  loadPhase4SignoffState,
  phase4VerificationCount,
  savePhase4SignoffState,
  type ControlRoomPhase4SignoffState,
} from '@/lib/control-room/controlRoomPhase4Delivery'
import { loadPhase3SignoffState } from '@/lib/control-room/controlRoomPhase3Delivery'

export function ControlRoomPhase4SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const phase3Signed = loadPhase3SignoffState().signedOffAt != null
  const [state, setState] = useState<ControlRoomPhase4SignoffState>(() => loadPhase4SignoffState())
  ...
  const persist = useCallback((next: ControlRoomPhase4SignoffState) => {
    setState(next)
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Control Room Phase 4 Mission timeline — Owner UI sign-off',
    })
  ...
  return (
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Phase 4 · Mission timeline sign-off</p>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {!phase3Signed && (
            <p className="m-0 text-[var(--text-dense-meta)] text-[var(--warning)]">
              Sign off Phase 3 (Command intent) before verifying Phase 4 items.
            </p>
          )}
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Verify mission timeline, Audit links, nightly drift summary, and Agent trajectory replay in the
            diagnosis zone, then sign off to unlock Phase 5 (Promote / cutover). Stored in local storage (v
            {CONTROL_ROOM_PHASE4_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {CONTROL_ROOM_PHASE4_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">Phase 4 signed off</DenseTag>
                {canAdmin && (
      ...
      <ConfirmDialog
        ...
        title="Sign off Phase 4 delivery"
        message="Confirm mission timeline, Audit links, nightly drift summary, and Agent trajectory replay behave as specified. Phase 5 (Promote / cutover) can proceed after sign-off."
