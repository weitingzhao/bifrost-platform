...
import {
  ...
  CONTROL_ROOM_PHASE2_DELIVERY_ITEMS,
  CONTROL_ROOM_PHASE2_VERSION,
  loadPhase2SignoffState,
  phase2VerificationCount,
  savePhase2SignoffState,
  type ControlRoomPhase2SignoffState,
} from '@/lib/control-room/controlRoomPhase2Delivery'
import { loadPhase1SignoffState } from '@/lib/control-room/controlRoomPhase1Delivery'

export function ControlRoomPhase2SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const phase1Signed = loadPhase1SignoffState().signedOffAt != null
  const [state, setState] = useState<ControlRoomPhase2SignoffState>(() => loadPhase2SignoffState())
  ...
  const persist = useCallback((next: ControlRoomPhase2SignoffState) => {
    setState(next)
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Control Room Phase 2 Payload depth — Owner UI sign-off',
    })
  ...
  return (
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Phase 2 · Payload depth sign-off</p>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {!phase1Signed && (
            <p className="m-0 text-[var(--text-dense-meta)] text-[var(--warning)]">
              Sign off Phase 1 (Operate Loop) before verifying Phase 2 items.
            </p>
          )}
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Verify Trade readiness, dev/prod comparison, and coupling hints in the Payload diagnosis zone, then sign off
            to unlock Phase 3 (Command intent). Stored in local storage (v{CONTROL_ROOM_PHASE2_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {CONTROL_ROOM_PHASE2_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">Phase 2 signed off</DenseTag>
                {canAdmin && (
      ...
      <ConfirmDialog
        ...
        message="Confirm Trade readiness rows, dev/prod comparison, coupling gate hint, and Runtime Map drill-down behave as specified. Phase 3 (Command intent) can proceed after sign-off."
