...
import {
  ...
  CONTROL_ROOM_PHASE3_DELIVERY_ITEMS,
  CONTROL_ROOM_PHASE3_VERSION,
  loadPhase3SignoffState,
  phase3VerificationCount,
  savePhase3SignoffState,
  type ControlRoomPhase3SignoffState,
} from '@/lib/control-room/controlRoomPhase3Delivery'
import { loadPhase2SignoffState } from '@/lib/control-room/controlRoomPhase2Delivery'

export function ControlRoomPhase3SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const phase2Signed = loadPhase2SignoffState().signedOffAt != null
  const [state, setState] = useState<ControlRoomPhase3SignoffState>(() => loadPhase3SignoffState())
  const [expandedId, setExpandedId] = useState<string | null>(null)
  const [signConfirmOpen, setSignConfirmOpen] = useState(false)
  const [panelExpanded, setPanelExpanded] = useState(() => state.signedOffAt == null)

  const counts = phase3VerificationCount(state)
  const allVerified = allPhase3ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: ControlRoomPhase3SignoffState) => {
    setState(next)
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Control Room Phase 3 Command intent — Owner UI sign-off',
    })
  ...
  return (
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Phase 3 · Command intent sign-off</p>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {!phase2Signed && (
            <p className="m-0 text-[var(--text-dense-meta)] text-[var(--warning)]">
              Sign off Phase 2 (Payload depth) before verifying Phase 3 items.
            </p>
          )}
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Verify mission-scoped command intents, context-aware chips, Agent Desk prefill, and copy packs in the
            diagnosis zone, then sign off to unlock Phase 4 (Mission timeline). Stored in local storage (v
            {CONTROL_ROOM_PHASE3_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {CONTROL_ROOM_PHASE3_DELIVERY_ITEMS.map(item => {
      ...
      <ConfirmDialog
        ...
        message="Confirm command intent strip, context-aware chips, Agent Desk prefill, and copy packs behave as specified. Phase 4 (Mission timeline) can proceed after sign-off."
