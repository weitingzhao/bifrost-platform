...
export function PlatformDeliverActuatePanel({ target, hideActions }: PlatformDeliverActuatePanelProps) {
  ...
  const deliverMutation = useMutation({
    ...
    onSuccess: data => {
      ...
      setActionSuccess(
        runName
          ? `PipelineRun ${runName} started — open Rocket → Trade Release → History to watch progress.`
          : 'Pipeline run started — open Rocket → Trade Release → History to watch progress.',
      )
      if (runName) {
            ...
            <div className="flex flex-wrap items-start gap-2">
              ...
              {!cmAllOk && (
                <span className="text-dense-meta text-muted-foreground">
                  Rocket → Trade Release → Refresh Dockerfile CMs (includes platform CMs).
                </span>
              )}
            </div>
