...
export function LaneDetailCollapse({
  ...
  return (
    <details
      className="group rounded-[var(--card-radius)] border border-[var(--card-border)] bg-[var(--card-fill)]"
      open={open}
    ...
    >
      ...
      <div className={cn('border-t border-[var(--table-rule)]', bodyClassName)}>{children}</div>
    </details>
  )
}
