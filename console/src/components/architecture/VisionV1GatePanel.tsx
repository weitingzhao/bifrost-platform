...
import { fetchVisionV1Gate, runVisionV1Gate, signVisionV1, VISION_PROGRAM_GATES_QUERY_KEY } from '@/api/vision'
...
export function VisionV1GatePanel() {
  ...
  const gateQuery = useQuery({
    queryKey: ['vision', 'v1', 'gate'],
    queryFn: fetchVisionV1Gate,
    refetchInterval: 30_000,
  })

  const runMutation = useMutation({
    mutationFn: runVisionV1Gate,
  ...
  const signMutation = useMutation({
    mutationFn: () => signVisionV1('Vision V1 dev inner-loop on K3s — Owner sign-off'),
  ...
  return (
    <OpsSection
      ...
      description="bifrost-dev namespace on K3s; Mac Pro runs Vite + one local API; VITE_API_* template points remaining domains to dev gateway :30882."
      actions={
        canAdmin ? (
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {runMutation.isPending ? 'Running…' : 'Run V1 gate'}
            </Button>
            <Button
              size="sm"
            ...
            >
              {signMutation.isPending ? 'Signing…' : signed ? 'Signed' : 'Sign off V1'}
            </Button>
          </div>
        ) : undefined
      }
...
function GateChecksTable({ gate, loading }: { gate?: VisionV1GateResponse; loading: boolean }) {
  ...
  if (checks.length === 0) {
    return <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">Run V1 gate to populate checks.</p>
  }
  return (
    <DenseDataTable>
      <DenseTableHeader>
