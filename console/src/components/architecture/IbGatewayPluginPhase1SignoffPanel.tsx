...
import {
  allIbGatewayPluginPhase1ItemsVerified,
  IB_GATEWAY_PLUGIN_PHASE1_DELIVERY_ITEMS,
  IB_GATEWAY_PLUGIN_PHASE1_VERSION,
  loadIbGatewayPluginPhase1SignoffState,
  priorIbGatewayPluginPhase1Prerequisites,
  saveIbGatewayPluginPhase1SignoffState,
  ibGatewayPluginPhase1VerificationCount,
  type IbGatewayPluginPhase1SignoffState,
} from '@/lib/architecture/ibGatewayPluginPhase1Delivery'
import { notifyGovernanceSignoffChanged } from '@/lib/architecture/governanceSignoffEvents'

export function IbGatewayPluginPhase1SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const prior = priorIbGatewayPluginPhase1Prerequisites()
  const [state, setState] = useState<IbGatewayPluginPhase1SignoffState>(() =>
    loadIbGatewayPluginPhase1SignoffState(),
  )
  ...
  const counts = ibGatewayPluginPhase1VerificationCount(state)
  const allVerified = allIbGatewayPluginPhase1ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: IbGatewayPluginPhase1SignoffState) => {
    setState(next)
    saveIbGatewayPluginPhase1SignoffState(next)
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'IB Gateway Plugin Phase 1 — Gateway core + K8s deploy — Owner UI sign-off',
    })
    ...
    return (
      <section className="page-section panel-elevated px-4 py-3 opacity-70">
        <p className="briefing-section-kicker m-0 mb-1">
          IB Gateway Plugin Phase 1 · Gateway core sign-off
        </p>
        <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
          Complete Phase 0 sign-off first: {prior.missing.join('; ')}
        </p>
      </section>
    )
  }
    ...
    <section className="page-section panel-elevated px-4 py-3">
      ...
      {panelExpanded && (
        <div className="mt-3 flex flex-col gap-3">
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            IB Gateway Pod @ data NS — writes tick/account/operator to redis-ib. Default mock mode for
            cluster verify; patch ConfigMap to live when TWS is ready (v{IB_GATEWAY_PLUGIN_PHASE1_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {IB_GATEWAY_PLUGIN_PHASE1_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">Phase 1 signed off — proceed to Phase 2 Platform API</DenseTag>
                {canAdmin && (
              ...
              <>
                ...
                >
                  Sign off IB Gateway Plugin Phase 1
                </Button>
                {!allVerified && (
      ...
      <ConfirmDialog
        ...
        title="Sign off IB Gateway Plugin Phase 1"
        message="Confirm IB Gateway core is deployed, mock/live pipelines verified on redis-ib, and operator RPC responds. Phase 2 adds platform-api integration."
