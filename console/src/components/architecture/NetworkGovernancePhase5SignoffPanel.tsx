...
import {
  allNetworkGovernancePhase5ItemsVerified,
  NETWORK_GOVERNANCE_PHASE5_DELIVERY_ITEMS,
  NETWORK_GOVERNANCE_PHASE5_VERSION,
  loadNetworkGovernancePhase5SignoffState,
  networkGovernancePhase5VerificationCount,
  saveNetworkGovernancePhase5SignoffState,
  type NetworkGovernancePhase5SignoffState,
} from '@/lib/architecture/networkGovernancePhase5Delivery'

export function NetworkGovernancePhase5SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const [state, setState] = useState<NetworkGovernancePhase5SignoffState>(() =>
    loadNetworkGovernancePhase5SignoffState(),
  )
  ...
  const counts = networkGovernancePhase5VerificationCount(state)
  const allVerified = allNetworkGovernancePhase5ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: NetworkGovernancePhase5SignoffState) => {
    setState(next)
    saveNetworkGovernancePhase5SignoffState(next)
  }, [])
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Network Governance Phase 5 — Network Upgrade catalog — Owner UI sign-off',
    })
  ...
  return (
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Network Governance Phase 5 · Network Upgrade catalog sign-off</p>
      ...
      {panelExpanded && (
        <div className="mt-3 flex flex-col gap-3">
          ...
          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {NETWORK_GOVERNANCE_PHASE5_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">Network Phase 5 signed off</DenseTag>
                {canAdmin && (
              ...
              <>
                ...
                >
                  Sign off Network Phase 5 delivery
                </Button>
                {!allVerified && (
            ...
            {!canAdmin && !signed && (
              <span className="text-[var(--text-dense-caption)] text-[var(--muted-foreground)]">
                Admin token required to record Network Phase 5 sign-off.
              </span>
            )}
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        title="Sign off Network Governance Phase 5"
        message="Confirm Network Upgrade catalog reflects live deployment: FIREWALL_APPLIED, deployment progress, and answered research items aligned with spine D9."
