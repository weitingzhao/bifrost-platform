...
import {
  allIbGatewayPluginPhase4ItemsVerified,
  IB_GATEWAY_PLUGIN_PHASE4_DELIVERY_ITEMS,
  IB_GATEWAY_PLUGIN_PHASE4_VERSION,
  loadIbGatewayPluginPhase4SignoffState,
  priorIbGatewayPluginPhase4Prerequisites,
  saveIbGatewayPluginPhase4SignoffState,
  ibGatewayPluginPhase4VerificationCount,
  type IbGatewayPluginPhase4SignoffState,
} from '@/lib/architecture/ibGatewayPluginPhase4Delivery'
import { notifyGovernanceSignoffChanged } from '@/lib/architecture/governanceSignoffEvents'

export function IbGatewayPluginPhase4SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const prior = priorIbGatewayPluginPhase4Prerequisites()
  const [state, setState] = useState<IbGatewayPluginPhase4SignoffState>(() =>
    loadIbGatewayPluginPhase4SignoffState(),
  )
  ...
  const counts = ibGatewayPluginPhase4VerificationCount(state)
  const allVerified = allIbGatewayPluginPhase4ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: IbGatewayPluginPhase4SignoffState) => {
    setState(next)
    saveIbGatewayPluginPhase4SignoffState(next)
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'IB Gateway Plugin Phase 4 — Live TWS cutover — Owner UI sign-off',
    })
    ...
    return (
      <section className="page-section panel-elevated px-4 py-3 opacity-70">
        <p className="briefing-section-kicker m-0 mb-1">
          IB Gateway Plugin Phase 4 · Live TWS sign-off
        </p>
    ...
    <section className="page-section panel-elevated px-4 py-3">
      ...
      {panelExpanded && (
        <div className="mt-3 flex flex-col gap-3">
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Switch ib-gateway to live TWS @ Host (.30) + Secondary (.32) — final program phase (v
            {IB_GATEWAY_PLUGIN_PHASE4_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {IB_GATEWAY_PLUGIN_PHASE4_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">IB Gateway Plugin program complete — live TWS operational</DenseTag>
                {canAdmin && (
              ...
              <>
                ...
                >
                  Sign off IB Gateway Plugin Phase 4
                </Button>
                {!allVerified && (
      ...
      <ConfirmDialog
        ...
        title="Sign off IB Gateway Plugin Phase 4"
        message="Confirm ib-gateway runs in live mode with real TWS connections (Host + Secondary). This completes the IB Gateway Plugin program."
