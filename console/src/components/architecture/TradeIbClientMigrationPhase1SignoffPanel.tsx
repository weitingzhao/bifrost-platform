...
import {
  allTradeIbClientMigrationPhase1ItemsVerified,
  TRADE_IB_CLIENT_MIGRATION_PHASE1_DELIVERY_ITEMS,
  TRADE_IB_CLIENT_MIGRATION_PHASE1_VERSION,
  loadTradeIbClientMigrationPhase1SignoffState,
  priorTradeIbClientMigrationPhase1Prerequisites,
  saveTradeIbClientMigrationPhase1SignoffState,
  tradeIbClientMigrationPhase1VerificationCount,
  type TradeIbClientMigrationPhase1SignoffState,
} from '@/lib/architecture/tradeIbClientMigrationPhase1Delivery'
import { notifyGovernanceSignoffChanged } from '@/lib/architecture/governanceSignoffEvents'

export function TradeIbClientMigrationPhase1SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const prior = priorTradeIbClientMigrationPhase1Prerequisites()
  const [state, setState] = useState<TradeIbClientMigrationPhase1SignoffState>(() =>
    loadTradeIbClientMigrationPhase1SignoffState(),
  )
  ...
  const counts = tradeIbClientMigrationPhase1VerificationCount(state)
  const allVerified = allTradeIbClientMigrationPhase1ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: TradeIbClientMigrationPhase1SignoffState) => {
    setState(next)
    saveTradeIbClientMigrationPhase1SignoffState(next)
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Trade IB Client Migration Phase 1 — Gateway RPC parity — Owner UI sign-off',
    })
  ...
  return (
    <section id="phase-1-signoff" className="page-section panel-elevated px-4 py-3">
      <button
        type="button"
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Phase 1 · Gateway RPC parity sign-off</p>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {!prior.ok && (
            <p className="m-0 rounded-md border border-[var(--border)] bg-[var(--secondary)] px-3 py-2 text-[var(--text-dense-meta)] text-[var(--danger)]">
              Required prerequisite: {prior.missing.join('; ')}. Complete Phase 0 sign-off first.
            </p>
          )}

          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Platform Gateway implements all 9 operator RPC ops (live + mock). Run{' '}
            <code className="font-mono text-dense-caption">make verify-ib-gateway-rpc-parity</code>{' '}
            on cluster after image rollout. v{TRADE_IB_CLIENT_MIGRATION_PHASE1_VERSION}.
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {TRADE_IB_CLIENT_MIGRATION_PHASE1_DELIVERY_ITEMS.map(item => {
            ...
            {signed ? (
              <>
                <DenseTag variant="success">
                  Phase 1 signed off — proceed to TIBM2 health derivation
                </DenseTag>
                {canAdmin && (
      ...
      <ConfirmDialog
        ...
        title="Sign off Trade IB Migration Phase 1"
        message="Confirm Platform IB Gateway implements all 9 operator RPC ops. This unlocks TIBM2 (Trade health derivation from Platform gateway)."
