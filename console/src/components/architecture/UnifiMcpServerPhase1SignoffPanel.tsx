...
import {
  allUnifiMcpServerPhase1ItemsVerified,
  UNIFI_MCP_SERVER_PHASE1_DELIVERY_ITEMS,
  UNIFI_MCP_SERVER_PHASE1_VERSION,
  loadUnifiMcpServerPhase1SignoffState,
  priorUnifiMcpServerPrerequisites,
  saveUnifiMcpServerPhase1SignoffState,
  unifiMcpServerPhase1VerificationCount,
  type UnifiMcpServerPhase1SignoffState,
} from '@/lib/architecture/unifiMcpServerPhase1Delivery'
import { notifyGovernanceSignoffChanged } from '@/lib/architecture/governanceSignoffEvents'

export function UnifiMcpServerPhase1SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const prior = priorUnifiMcpServerPrerequisites()
  const [state, setState] = useState<UnifiMcpServerPhase1SignoffState>(() =>
    loadUnifiMcpServerPhase1SignoffState(),
  )
  ...
  const counts = unifiMcpServerPhase1VerificationCount(state)
  const allVerified = allUnifiMcpServerPhase1ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: UnifiMcpServerPhase1SignoffState) => {
    setState(next)
    saveUnifiMcpServerPhase1SignoffState(next)
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'UniFi MCP Server Phase 1 — REST client library — Owner UI sign-off',
    })
    ...
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">
          UniFi MCP Server Phase 1 · REST client library sign-off
        </p>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {!prior.ok && (
            <p className="m-0 text-[var(--text-dense-meta)] text-[var(--warning)]">
              Complete Network Governance program (Phase 8) before verifying UniFi MCP Server Phase 1.
              {prior.missing.length > 0 && ` Missing: ${prior.missing.join(', ')}.`}
            </p>
          )}
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Spine stream ① — shared Go client at api/internal/network/unifi (Session v2 per D9). No
            /api/v1/network/* routes yet; Phase 2 adds handlers + MCP read tools (v
            {UNIFI_MCP_SERVER_PHASE1_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {UNIFI_MCP_SERVER_PHASE1_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">UniFi MCP Server Phase 1 signed off — proceed to Phase 2</DenseTag>
                {canAdmin && (
              ...
              <>
                ...
                >
                  Sign off UniFi MCP Server Phase 1
                </Button>
                {!prior.ok && (
                  <span className="text-[var(--text-dense-caption)] text-[var(--muted-foreground)]">
                    Complete Network Governance Phase 8 first.
                  </span>
                )}
      ...
      <ConfirmDialog
        ...
        title="Sign off UniFi MCP Server Phase 1"
        message="Confirm the Go UniFi REST client library is delivered and tested. This sign-off covers the client package only — platform-api routes and MCP read tools follow in Phase 2."
