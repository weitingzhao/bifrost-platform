...
import { fetchVisionV5Gate, runVisionV5Gate, signVisionV5 } from '@/api/platform'
import type { VisionV1GateResponse } from '@/api/types'
import { OpsSection, OpsSubsectionTitle } from '@/components/layout/OpsSection'
import { usePlatformAuth } from '@/hooks/usePlatformAuth'

export function VisionV5GatePanel() {
  ...
  const gateQuery = useQuery({
    queryKey: ['vision', 'v5', 'gate'],
    queryFn: fetchVisionV5Gate,
    refetchInterval: 30_000,
  })

  const runMutation = useMutation({
    mutationFn: runVisionV5Gate,
    onMutate: () => setRunError(null),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['vision', 'v5', 'gate'] })
  ...
  const signMutation = useMutation({
    mutationFn: () => signVisionV5('Vision V5 full convergence — Dual Flywheel complete — Owner sign-off'),
  ...
  return (
    <OpsSection
      ...
      description="Dev + Ops + Business Agents unified in one Cursor window. Ollama for sensitive analysis; trade insights feed platform; L3 via PR."
      actions={
        canAdmin ? (
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {runMutation.isPending ? 'Running…' : 'Run V5 gate'}
            </Button>
            <Button
              size="sm"
            ...
            >
              {signMutation.isPending ? 'Signing…' : signed ? 'Signed' : 'Sign off V5'}
            </Button>
          </div>
        ) : undefined
      }
    ...
    >
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        {signed && (
          <DenseTag variant="success">Dual Flywheel complete</DenseTag>
        )}
        {gate?.detail != null && (
...
function GateChecksTable({ gate, loading }: { gate?: VisionV1GateResponse; loading: boolean }) {
  ...
  if (checks.length === 0) {
    return <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">Run V5 gate to populate checks.</p>
  }
  return (
    <DenseDataTable>
      <DenseTableHeader>
