...
import {
  allGovernancePhase5ItemsVerified,
  GOVERNANCE_PHASE5_DELIVERY_ITEMS,
  GOVERNANCE_PHASE5_VERSION,
  governancePhase5VerificationCount,
  loadGovernancePhase5SignoffState,
  saveGovernancePhase5SignoffState,
  type GovernancePhase5SignoffState,
} from '@/lib/architecture/governancePhase5Delivery'
import { isGovernancePhase4SignedOff } from '@/lib/architecture/governancePhase4Delivery'

export function GovernancePhase5SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const phase4Signed = isGovernancePhase4SignedOff()
  const [state, setState] = useState<GovernancePhase5SignoffState>(() =>
    loadGovernancePhase5SignoffState(),
  )
  ...
  const counts = governancePhase5VerificationCount(state)
  const allVerified = allGovernancePhase5ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: GovernancePhase5SignoffState) => {
    setState(next)
    saveGovernancePhase5SignoffState(next)
  }, [])
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Governance Phase 5 Blueprint three-zone layout — Owner UI sign-off (program complete)',
    })
    ...
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">
          Governance Phase 5 · Blueprint three-zone layout sign-off
        </p>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {!phase4Signed && (
            <p className="m-0 text-[var(--text-dense-meta)] text-[var(--warning)]">
              Sign off Governance Phase 4 (Reconciliation extension) before verifying Phase 5 items.
            </p>
          )}
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Final governance boundary program phase — Constitution / Spine / Projection visual zones on
            Blueprint (v{GOVERNANCE_PHASE5_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {GOVERNANCE_PHASE5_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">Governance program signed off</DenseTag>
                {canAdmin && (
      ...
      <ConfirmDialog
        ...
        title="Sign off Governance Phase 5"
        message="Confirm Blueprint shows Constitution, Spine, and Projection as three distinct zones with jump navigation and Copy Prompt section order matching governance layers. This completes the governance boundary program."
