...
import {
  allTradeIbClientMigrationPhase0ItemsVerified,
  TRADE_IB_CLIENT_MIGRATION_PHASE0_DELIVERY_ITEMS,
  TRADE_IB_CLIENT_MIGRATION_PHASE0_VERSION,
  loadTradeIbClientMigrationPhase0SignoffState,
  priorTradeIbClientMigrationPhase0Prerequisites,
  saveTradeIbClientMigrationPhase0SignoffState,
  tradeIbClientMigrationPhase0VerificationCount,
  type TradeIbClientMigrationPhase0SignoffState,
} from '@/lib/architecture/tradeIbClientMigrationPhase0Delivery'
import { notifyGovernanceSignoffChanged } from '@/lib/architecture/governanceSignoffEvents'

export function TradeIbClientMigrationPhase0SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const prior = priorTradeIbClientMigrationPhase0Prerequisites()
  const [state, setState] = useState<TradeIbClientMigrationPhase0SignoffState>(() =>
    loadTradeIbClientMigrationPhase0SignoffState(),
  )
  ...
  const counts = tradeIbClientMigrationPhase0VerificationCount(state)
  const allVerified = allTradeIbClientMigrationPhase0ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: TradeIbClientMigrationPhase0SignoffState) => {
    setState(next)
    saveTradeIbClientMigrationPhase0SignoffState(next)
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Trade IB Client Migration Phase 0 — inventory & scope — Owner UI sign-off',
    })
  ...
  return (
    <section id="phase-0-signoff" className="page-section panel-elevated px-4 py-3">
      <button
        type="button"
      ...
      {panelExpanded && (
        <div className="mt-3 flex flex-col gap-3">
          {!prior.ok && (
            <p className="m-0 rounded-md border border-[var(--border)] bg-[var(--secondary)] px-3 py-2 text-[var(--text-dense-meta)] text-[var(--warning)]">
              Recommended prerequisite: {prior.missing.join('; ')}. You may still sign off Phase 0
              inventory; block TIBM1 until IBGP is stable.
            </p>
          )}

          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Code survey deliverable — confirm the surface inventory and RPC matrix above match your
            cluster before starting Gateway RPC parity (TIBM1). v{TRADE_IB_CLIENT_MIGRATION_PHASE0_VERSION}.
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {TRADE_IB_CLIENT_MIGRATION_PHASE0_DELIVERY_ITEMS.map(item => {
            ...
            {signed ? (
              <>
                <DenseTag variant="success">
                  Phase 0 signed off — proceed to TIBM1 Gateway RPC parity
                </DenseTag>
                {canAdmin && (
      ...
      <ConfirmDialog
        ...
        title="Sign off Trade IB Migration Phase 0"
        message="Confirm the IB surface inventory and phase roadmap are accurate. This unlocks TIBM1 implementation work (Gateway RPC parity)."
