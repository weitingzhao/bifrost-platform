...
import {
  allTradeIbClientMigrationPhase4ItemsVerified,
  TRADE_IB_CLIENT_MIGRATION_PHASE4_DELIVERY_ITEMS,
  TRADE_IB_CLIENT_MIGRATION_PHASE4_VERSION,
  loadTradeIbClientMigrationPhase4SignoffState,
  priorTradeIbClientMigrationPhase4Prerequisites,
  saveTradeIbClientMigrationPhase4SignoffState,
  tradeIbClientMigrationPhase4VerificationCount,
  type TradeIbClientMigrationPhase4SignoffState,
} from '@/lib/architecture/tradeIbClientMigrationPhase4Delivery'
import { notifyGovernanceSignoffChanged } from '@/lib/architecture/governanceSignoffEvents'

export function TradeIbClientMigrationPhase4SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const prior = priorTradeIbClientMigrationPhase4Prerequisites()
  const [state, setState] = useState<TradeIbClientMigrationPhase4SignoffState>(() =>
    loadTradeIbClientMigrationPhase4SignoffState(),
  )
  ...
  const counts = tradeIbClientMigrationPhase4VerificationCount(state)
  const allVerified = allTradeIbClientMigrationPhase4ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: TradeIbClientMigrationPhase4SignoffState) => {
    setState(next)
    saveTradeIbClientMigrationPhase4SignoffState(next)
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Trade IB Client Migration Phase 4 — UI/legacy cleanup — Owner UI sign-off',
    })
  ...
  return (
    <section id="phase-4-signoff" className="page-section panel-elevated px-4 py-3">
      <button
        type="button"
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Phase 4 · UI + legacy cleanup sign-off</p>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {!prior.ok && (
            <p className="m-0 rounded-md border border-[var(--border)] bg-[var(--secondary)] px-3 py-2 text-[var(--text-dense-meta)] text-[var(--danger)]">
              Required prerequisite: {prior.missing.join('; ')}. Complete Phase 3 sign-off first.
            </p>
          )}

          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Trade Frontend + Ops API relabel IB ingest as Platform IB Gateway @ redis-ib — no legacy
            trade-socket STS references in active UI. Run{' '}
            <code className="font-mono text-dense-caption">make verify-trade-ib-ui</code> on cluster.
            v{TRADE_IB_CLIENT_MIGRATION_PHASE4_VERSION}.
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {TRADE_IB_CLIENT_MIGRATION_PHASE4_DELIVERY_ITEMS.map(item => {
            ...
            {signed ? (
              <>
                <DenseTag variant="success">
                  Phase 4 signed off — Trade IB Client Migration program complete
                </DenseTag>
                {canAdmin && (
      ...
      <ConfirmDialog
        ...
        title="Sign off Trade IB Migration Phase 4"
        message="Confirm Trade Frontend and Ops market ingest UI reflect Platform IB Gateway @ redis-ib. This completes the Trade IB Client Migration program (TIBM0–TIBM4)."
