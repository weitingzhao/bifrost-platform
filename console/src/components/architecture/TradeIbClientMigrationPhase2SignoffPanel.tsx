...
import {
  allTradeIbClientMigrationPhase2ItemsVerified,
  TRADE_IB_CLIENT_MIGRATION_PHASE2_DELIVERY_ITEMS,
  TRADE_IB_CLIENT_MIGRATION_PHASE2_VERSION,
  loadTradeIbClientMigrationPhase2SignoffState,
  priorTradeIbClientMigrationPhase2Prerequisites,
  saveTradeIbClientMigrationPhase2SignoffState,
  tradeIbClientMigrationPhase2VerificationCount,
  type TradeIbClientMigrationPhase2SignoffState,
} from '@/lib/architecture/tradeIbClientMigrationPhase2Delivery'
import { notifyGovernanceSignoffChanged } from '@/lib/architecture/governanceSignoffEvents'

export function TradeIbClientMigrationPhase2SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const prior = priorTradeIbClientMigrationPhase2Prerequisites()
  const [state, setState] = useState<TradeIbClientMigrationPhase2SignoffState>(() =>
    loadTradeIbClientMigrationPhase2SignoffState(),
  )
  ...
  const counts = tradeIbClientMigrationPhase2VerificationCount(state)
  const allVerified = allTradeIbClientMigrationPhase2ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: TradeIbClientMigrationPhase2SignoffState) => {
    setState(next)
    saveTradeIbClientMigrationPhase2SignoffState(next)
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Trade IB Client Migration Phase 2 — Health derivation — Owner UI sign-off',
    })
  ...
  return (
    <section id="phase-2-signoff" className="page-section panel-elevated px-4 py-3">
      <button
        type="button"
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Phase 2 · Health derivation sign-off</p>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {!prior.ok && (
            <p className="m-0 rounded-md border border-[var(--border)] bg-[var(--secondary)] px-3 py-2 text-[var(--text-dense-meta)] text-[var(--danger)]">
              Required prerequisite: {prior.missing.join('; ')}. Complete Phase 1 sign-off first.
            </p>
          )}

          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Platform IB Gateway health derives from redis-ib (daemon + Monitor status) (live + mock). Run{' '}
            <code className="font-mono text-dense-caption">make verify-trade-ib-health</code>{' '}
            on cluster after image rollout. v{TRADE_IB_CLIENT_MIGRATION_PHASE2_VERSION}.
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {TRADE_IB_CLIENT_MIGRATION_PHASE2_DELIVERY_ITEMS.map(item => {
            ...
            {signed ? (
              <>
                <DenseTag variant="success">
                  Phase 2 signed off — proceed to TIBM3 Celery RPC-only
                </DenseTag>
                {canAdmin && (
      ...
      <ConfirmDialog
        ...
        title="Sign off Trade IB Migration Phase 2"
        message="Confirm daemon + Monitor status derive IB health from Platform IB Gateway @ redis-ib. This unlocks TIBM3 (Celery bars RPC-only)."
