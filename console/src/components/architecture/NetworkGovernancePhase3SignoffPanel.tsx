...
import {
  allNetworkGovernancePhase3ItemsVerified,
  NETWORK_GOVERNANCE_PHASE3_DELIVERY_ITEMS,
  NETWORK_GOVERNANCE_PHASE3_VERSION,
  loadNetworkGovernancePhase3SignoffState,
  networkGovernancePhase3VerificationCount,
  saveNetworkGovernancePhase3SignoffState,
  type NetworkGovernancePhase3SignoffState,
} from '@/lib/architecture/networkGovernancePhase3Delivery'

export function NetworkGovernancePhase3SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const [state, setState] = useState<NetworkGovernancePhase3SignoffState>(() =>
    loadNetworkGovernancePhase3SignoffState(),
  )
  ...
  const counts = networkGovernancePhase3VerificationCount(state)
  const allVerified = allNetworkGovernancePhase3ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: NetworkGovernancePhase3SignoffState) => {
    setState(next)
    saveNetworkGovernancePhase3SignoffState(next)
  }, [])
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Network Governance Phase 3 — Spine extension (D9, stream, coupling) — Owner UI sign-off',
    })
  ...
  return (
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Network Governance Phase 3 · Spine extension sign-off</p>
      ...
      {panelExpanded && (
        <div className="mt-3 flex flex-col gap-3">
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Verify Spine updates in config/ops-context.yaml (decision D9, network-upgrade-core 5/6, coupling
            unifi_session_v2). Confirm on Program / Briefing pages, then sign off (v{NETWORK_GOVERNANCE_PHASE3_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {NETWORK_GOVERNANCE_PHASE3_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">Network Phase 3 signed off</DenseTag>
                {canAdmin && (
              ...
              <>
                ...
                >
                  Sign off Network Phase 3 delivery
                </Button>
                {!allVerified && (
            ...
            {!canAdmin && !signed && (
              <span className="text-[var(--text-dense-caption)] text-[var(--muted-foreground)]">
                Admin token required to record Network Phase 3 sign-off.
              </span>
            )}
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        title="Sign off Network Governance Phase 3"
        message="Confirm Spine extension: decision D9 (Session v2), network-upgrade-core 5/6 with firewall note, and coupling surface unifi_session_v2."
