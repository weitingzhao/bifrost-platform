...
import {
  allNetworkGovernancePhase1ItemsVerified,
  NETWORK_GOVERNANCE_PHASE1_DELIVERY_ITEMS,
  NETWORK_GOVERNANCE_PHASE1_VERSION,
  loadNetworkGovernancePhase1SignoffState,
  networkGovernancePhase1VerificationCount,
  saveNetworkGovernancePhase1SignoffState,
  type NetworkGovernancePhase1SignoffState,
} from '@/lib/architecture/networkGovernancePhase1Delivery'

export function NetworkGovernancePhase1SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const [state, setState] = useState<NetworkGovernancePhase1SignoffState>(() =>
    loadNetworkGovernancePhase1SignoffState(),
  )
  ...
  const counts = networkGovernancePhase1VerificationCount(state)
  const allVerified = allNetworkGovernancePhase1ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: NetworkGovernancePhase1SignoffState) => {
    setState(next)
    saveNetworkGovernancePhase1SignoffState(next)
  }, [])
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Network Governance Phase 1 — Constitution network extension — Owner UI sign-off',
    })
    ...
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">
          Network Governance Phase 1 · Constitution extension sign-off
        </p>
      ...
      {panelExpanded && (
        <div className="mt-3 flex flex-col gap-3">
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Verify network semantics in Blueprint Constitution (Principle 8, L0/L1/L2, Strategy C, Owner exceptions,
            Success criteria, Actuation phases), then sign off (v{NETWORK_GOVERNANCE_PHASE1_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {NETWORK_GOVERNANCE_PHASE1_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">Network Phase 1 signed off</DenseTag>
                {canAdmin && (
              ...
              <>
                ...
                >
                  Sign off Network Phase 1 delivery
                </Button>
                {!allVerified && (
            ...
            {!canAdmin && !signed && (
              <span className="text-[var(--text-dense-caption)] text-[var(--muted-foreground)]">
                Admin token required to record Network Phase 1 sign-off.
              </span>
            )}
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        title="Sign off Network Governance Phase 1"
        message="Confirm Blueprint Constitution network extension: Principle 8, authorization levels, Strategy C UniFi, Owner exceptions, Network success criterion, and actuation phase overlays."
