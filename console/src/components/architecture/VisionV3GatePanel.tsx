...
import { fetchVisionV3Gate, runVisionV3Gate, signVisionV3 } from '@/api/platform'
import type { VisionV1GateResponse } from '@/api/types'
import { OpsSection, OpsSubsectionTitle } from '@/components/layout/OpsSection'
import { usePlatformAuth } from '@/hooks/usePlatformAuth'

export function VisionV3GatePanel() {
  ...
  const gateQuery = useQuery({
    queryKey: ['vision', 'v3', 'gate'],
    queryFn: fetchVisionV3Gate,
    refetchInterval: 30_000,
  })

  const runMutation = useMutation({
    mutationFn: runVisionV3Gate,
    onMutate: () => setRunError(null),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['vision', 'v3', 'gate'] })
  ...
  const signMutation = useMutation({
    mutationFn: () => signVisionV3('Vision V3 Ops Agent L1/L2 — Owner sign-off'),
  ...
  return (
    <OpsSection
      ...
      description="MCP platform + K8s/Redis/PG bridges → Alertmanager webhook diagnosis → L1 actuation with audit. L2 requires Owner confirm."
      actions={
        canAdmin ? (
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {runMutation.isPending ? 'Running…' : 'Run V3 gate'}
            </Button>
            <Button
              size="sm"
            ...
            >
              {signMutation.isPending ? 'Signing…' : signed ? 'Signed' : 'Sign off V3'}
            </Button>
          </div>
        ) : undefined
      }
...
function GateChecksTable({ gate, loading }: { gate?: VisionV1GateResponse; loading: boolean }) {
  ...
  if (checks.length === 0) {
    return <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">Run V3 gate to populate checks.</p>
  }
  return (
    <DenseDataTable>
      <DenseTableHeader>
