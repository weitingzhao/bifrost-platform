...
import {
  allNetworkGovernancePhase6ItemsVerified,
  NETWORK_GOVERNANCE_PHASE6_DELIVERY_ITEMS,
  NETWORK_GOVERNANCE_PHASE6_VERSION,
  loadNetworkGovernancePhase6SignoffState,
  networkGovernancePhase6VerificationCount,
  saveNetworkGovernancePhase6SignoffState,
  type NetworkGovernancePhase6SignoffState,
} from '@/lib/architecture/networkGovernancePhase6Delivery'

export function NetworkGovernancePhase6SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const [state, setState] = useState<NetworkGovernancePhase6SignoffState>(() =>
    loadNetworkGovernancePhase6SignoffState(),
  )
  ...
  const counts = networkGovernancePhase6VerificationCount(state)
  const allVerified = allNetworkGovernancePhase6ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: NetworkGovernancePhase6SignoffState) => {
    setState(next)
    saveNetworkGovernancePhase6SignoffState(next)
  }, [])
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Network Governance Phase 6 — Control Room views — Owner UI sign-off',
    })
  ...
  return (
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Network Governance Phase 6 · Control Room views sign-off</p>
      ...
      {panelExpanded && (
        <div className="mt-3 flex flex-col gap-3">
          ...
          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {NETWORK_GOVERNANCE_PHASE6_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">Network Phase 6 signed off</DenseTag>
                {canAdmin && (
              ...
              <>
                ...
                >
                  Sign off Network Phase 6 delivery
                </Button>
                {!allVerified && (
            ...
            {!canAdmin && !signed && (
              <span className="text-[var(--text-dense-caption)] text-[var(--muted-foreground)]">
                Admin token required to record Network Phase 6 sign-off.
              </span>
            )}
          </div>
        </div>
