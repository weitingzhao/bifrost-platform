...
import {
  allIbGatewayPluginPhase3ItemsVerified,
  IB_GATEWAY_PLUGIN_PHASE3_DELIVERY_ITEMS,
  IB_GATEWAY_PLUGIN_PHASE3_VERSION,
  loadIbGatewayPluginPhase3SignoffState,
  priorIbGatewayPluginPhase3Prerequisites,
  saveIbGatewayPluginPhase3SignoffState,
  ibGatewayPluginPhase3VerificationCount,
  type IbGatewayPluginPhase3SignoffState,
} from '@/lib/architecture/ibGatewayPluginPhase3Delivery'
import { notifyGovernanceSignoffChanged } from '@/lib/architecture/governanceSignoffEvents'

export function IbGatewayPluginPhase3SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const prior = priorIbGatewayPluginPhase3Prerequisites()
  const [state, setState] = useState<IbGatewayPluginPhase3SignoffState>(() =>
    loadIbGatewayPluginPhase3SignoffState(),
  )
  ...
  const counts = ibGatewayPluginPhase3VerificationCount(state)
  const allVerified = allIbGatewayPluginPhase3ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: IbGatewayPluginPhase3SignoffState) => {
    setState(next)
    saveIbGatewayPluginPhase3SignoffState(next)
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'IB Gateway Plugin Phase 3 — Trade cutover — Owner UI sign-off',
    })
    ...
    return (
      <section className="page-section panel-elevated px-4 py-3 opacity-70">
        <p className="briefing-section-kicker m-0 mb-1">
          IB Gateway Plugin Phase 3 · Trade cutover sign-off
        </p>
    ...
    <section className="page-section panel-elevated px-4 py-3">
      ...
      {panelExpanded && (
        <div className="mt-3 flex flex-col gap-3">
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Legacy trade-socket IB retired — Trade reads Platform redis-ib bus (v
            {IB_GATEWAY_PLUGIN_PHASE3_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {IB_GATEWAY_PLUGIN_PHASE3_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">Phase 3 complete — proceed to Phase 4 live TWS</DenseTag>
                {canAdmin && (
              ...
              <>
                ...
                >
                  Sign off IB Gateway Plugin Phase 3
                </Button>
                {!allVerified && (
      ...
      <ConfirmDialog
        ...
        title="Sign off IB Gateway Plugin Phase 3"
        message="Confirm legacy trade-socket IB is retired and Trade workloads read the Platform redis-ib bus. Live TWS mode remains a separate Owner decision after program sign-off."
