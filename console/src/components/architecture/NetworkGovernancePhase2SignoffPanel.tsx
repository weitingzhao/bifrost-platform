...
import {
  allNetworkGovernancePhase2ItemsVerified,
  NETWORK_GOVERNANCE_PHASE2_DELIVERY_ITEMS,
  NETWORK_GOVERNANCE_PHASE2_VERSION,
  loadNetworkGovernancePhase2SignoffState,
  networkGovernancePhase2VerificationCount,
  saveNetworkGovernancePhase2SignoffState,
  type NetworkGovernancePhase2SignoffState,
} from '@/lib/architecture/networkGovernancePhase2Delivery'

export function NetworkGovernancePhase2SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const [state, setState] = useState<NetworkGovernancePhase2SignoffState>(() =>
    loadNetworkGovernancePhase2SignoffState(),
  )
  ...
  const counts = networkGovernancePhase2VerificationCount(state)
  const allVerified = allNetworkGovernancePhase2ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: NetworkGovernancePhase2SignoffState) => {
    setState(next)
    saveNetworkGovernancePhase2SignoffState(next)
  }, [])
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Network Governance Phase 2 — AI Platform capabilities extension — Owner UI sign-off',
    })
    ...
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">
          Network Governance Phase 2 · AI Platform capabilities sign-off
        </p>
      ...
      {panelExpanded && (
        <div className="mt-3 flex flex-col gap-3">
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Verify network extensions in AI Platform capabilities, success criteria, and boundaries (Constitution
            zone), then sign off (v{NETWORK_GOVERNANCE_PHASE2_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {NETWORK_GOVERNANCE_PHASE2_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">Network Phase 2 signed off</DenseTag>
                {canAdmin && (
              ...
              <>
                ...
                >
                  Sign off Network Phase 2 delivery
                </Button>
                {!allVerified && (
            ...
            {!canAdmin && !signed && (
              <span className="text-[var(--text-dense-caption)] text-[var(--muted-foreground)]">
                Admin token required to record Network Phase 2 sign-off.
              </span>
            )}
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        title="Sign off Network Governance Phase 2"
        message="Confirm AI Platform network extension: Discovery/Maintenance/Repair examples, Network success criterion, and Network Security Posture boundary."
