...
import {
  allGovernancePhase4ItemsVerified,
  GOVERNANCE_PHASE4_DELIVERY_ITEMS,
  GOVERNANCE_PHASE4_VERSION,
  governancePhase4VerificationCount,
  loadGovernancePhase4SignoffState,
  saveGovernancePhase4SignoffState,
  type GovernancePhase4SignoffState,
} from '@/lib/architecture/governancePhase4Delivery'
import { isGovernancePhase3SignedOff } from '@/lib/architecture/governancePhase3Delivery'

export function GovernancePhase4SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const phase3Signed = isGovernancePhase3SignedOff()
  const [state, setState] = useState<GovernancePhase4SignoffState>(() =>
    loadGovernancePhase4SignoffState(),
  )
  ...
  const counts = governancePhase4VerificationCount(state)
  const allVerified = allGovernancePhase4ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: GovernancePhase4SignoffState) => {
    setState(next)
    saveGovernancePhase4SignoffState(next)
  }, [])
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Governance Phase 4 Reconciliation extension — Owner UI sign-off',
    })
  ...
  return (
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Governance Phase 4 · Reconciliation extension sign-off</p>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {!phase3Signed && (
            <p className="m-0 text-[var(--text-dense-meta)] text-[var(--warning)]">
              Sign off Governance Phase 3 (Spine semantics) before verifying Phase 4 items.
            </p>
          )}
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Verify catalog-spine parity gate, CATALOG_DRIFT UI, nightly scan_layer3, and CI milestone ref
            checks (v{GOVERNANCE_PHASE4_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {GOVERNANCE_PHASE4_DELIVERY_ITEMS.map(item => {
