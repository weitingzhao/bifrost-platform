...
import {
  allNetworkGovernancePhase4ItemsVerified,
  NETWORK_GOVERNANCE_PHASE4_DELIVERY_ITEMS,
  NETWORK_GOVERNANCE_PHASE4_VERSION,
  loadNetworkGovernancePhase4SignoffState,
  networkGovernancePhase4VerificationCount,
  saveNetworkGovernancePhase4SignoffState,
  type NetworkGovernancePhase4SignoffState,
} from '@/lib/architecture/networkGovernancePhase4Delivery'

export function NetworkGovernancePhase4SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const [state, setState] = useState<NetworkGovernancePhase4SignoffState>(() =>
    loadNetworkGovernancePhase4SignoffState(),
  )
  ...
  const counts = networkGovernancePhase4VerificationCount(state)
  const allVerified = allNetworkGovernancePhase4ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: NetworkGovernancePhase4SignoffState) => {
    setState(next)
    saveNetworkGovernancePhase4SignoffState(next)
  }, [])
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Network Governance Phase 4 — Agent Protocol extension — Owner UI sign-off',
    })
  ...
  return (
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Network Governance Phase 4 · Agent Protocol sign-off</p>
      ...
      {panelExpanded && (
        <div className="mt-3 flex flex-col gap-3">
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Verify Agent Protocol network extensions (forbidden actions, Ops mode, diagnostic playbooks) on Architecture
            → Agent Protocol, then sign off (v{NETWORK_GOVERNANCE_PHASE4_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {NETWORK_GOVERNANCE_PHASE4_DELIVERY_ITEMS.map(item => {
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-3">
            {signed ? (
              <>
                <DenseTag variant="success">Network Phase 4 signed off</DenseTag>
                {canAdmin && (
              ...
              <>
                ...
                >
                  Sign off Network Phase 4 delivery
                </Button>
                {!allVerified && (
            ...
            {!canAdmin && !signed && (
              <span className="text-[var(--text-dense-caption)] text-[var(--muted-foreground)]">
                Admin token required to record Network Phase 4 sign-off.
              </span>
            )}
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        title="Sign off Network Governance Phase 4"
        message="Confirm Agent Protocol network extension: forbidden posture actions, Ops mode L0/L1 network scope, and Network diagnostic playbooks."
