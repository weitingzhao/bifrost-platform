...
export function VisionV2GatePanel() {
  ...
  const gateQuery = useQuery({
    queryKey: ['vision', 'v2', 'gate'],
    queryFn: fetchVisionV2Gate,
    refetchInterval: 30_000,
  })

  const runMutation = useMutation({
    mutationFn: runVisionV2Gate,
  ...
  const signMutation = useMutation({
    mutationFn: () => signVisionV2('Vision V2 Dev Agent closed-loop — Owner sign-off'),
  ...
  return (
    <OpsSection
      ...
      description="Pre-push checks → Tekton deliver-stg → STG smoke verify → report. Promote uses release-gate before deliver-prod."
      actions={
        canAdmin ? (
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {runMutation.isPending ? 'Running…' : 'Run V2 gate'}
            </Button>
            <Button
              size="sm"
            ...
            >
              {signMutation.isPending ? 'Signing…' : signed ? 'Signed' : 'Sign off V2'}
            </Button>
          </div>
        ) : undefined
      }
...
function GateChecksTable({ gate, loading }: { gate?: VisionV1GateResponse; loading: boolean }) {
  ...
  if (checks.length === 0) {
    return <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">Run V2 gate to populate checks.</p>
  }
  return (
    <DenseDataTable>
      <DenseTableHeader>
