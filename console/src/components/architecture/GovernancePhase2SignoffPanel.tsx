...
import {
  allGovernancePhase2ItemsVerified,
  GOVERNANCE_PHASE2_DELIVERY_ITEMS,
  GOVERNANCE_PHASE2_VERSION,
  governancePhase2VerificationCount,
  loadGovernancePhase2SignoffState,
  saveGovernancePhase2SignoffState,
  type GovernancePhase2SignoffState,
} from '@/lib/architecture/governancePhase2Delivery'
import { isGovernancePhase1SignedOff } from '@/lib/architecture/governancePhase1Delivery'

export function GovernancePhase2SignoffPanel() {
  const { canAdmin, caps } = usePlatformAuth()
  const phase1Signed = isGovernancePhase1SignedOff()
  const [state, setState] = useState<GovernancePhase2SignoffState>(() =>
    loadGovernancePhase2SignoffState(),
  )
  ...
  const counts = governancePhase2VerificationCount(state)
  const allVerified = allGovernancePhase2ItemsVerified(state)
  const signed = state.signedOffAt != null

  const persist = useCallback((next: GovernancePhase2SignoffState) => {
    setState(next)
    saveGovernancePhase2SignoffState(next)
  }, [])
  ...
  function handleSignOff() {
    persist({
      ...
      note: 'Governance Phase 2 Projection placement — Owner UI sign-off',
    })
  ...
  return (
    <section className="page-section panel-elevated px-4 py-3">
      ...
      >
        ...
        <p className="briefing-section-kicker m-0">Governance Phase 2 · Projection placement sign-off</p>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {!phase1Signed && (
            <p className="m-0 text-[var(--text-dense-meta)] text-[var(--warning)]">
              Sign off Governance Phase 1 (Blueprint Constitution) before verifying Phase 2 items.
            </p>
          )}
          <p className="m-0 text-[var(--text-dense-meta)] text-[var(--muted-foreground)]">
            Verify spine-driven Delivery phases, purified environments-catalog, and Projection labels on
            Mission Control → Delivery Board → Blueprint (v{GOVERNANCE_PHASE2_VERSION}).
          </p>

          <ul className="m-0 flex list-none flex-col gap-2 p-0">
            {GOVERNANCE_PHASE2_DELIVERY_ITEMS.map(item => {
