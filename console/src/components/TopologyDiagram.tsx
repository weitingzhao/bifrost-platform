...
export function TopologyDiagram({ data }: { data: TopologyResponse }) {
  ...
  return (
    <section className="panel-elevated p-3 flex flex-col gap-3">
      <header className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <h2 className="m-0 text-sm font-semibold">Network topology</h2>
          <span className="badge">{data.label}</span>
          <span className="badge">phase: {data.deployment_phase}</span>
        </div>
        <div className="segment" role="tablist" aria-label="Role view">
          <button
            type="button"
            role="tab"
            data-active={roleView === 'compose'}
            onClick={() => setRoleView('compose')}
          >
            Compose roles
          </button>
          <button
            type="button"
            role="tab"
            data-active={roleView === 'k3s'}
            onClick={() => setRoleView('k3s')}
          >
            K3s target
          </button>
        </div>
        ...
        {data.nodes.map(node => (
          ...
          >
            ...
            {node.in_k3s_cluster && roleView === 'k3s' && (
              <span className="badge mt-1 text-[10px]">in K3s cluster</span>
            )}
