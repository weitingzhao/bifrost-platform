...
function buildReleasePrompt(req: StartRunRequest): string {
  ...
  const lines: string[] = [
    ...
    '7. Call get_delivery_revisions with repos="bifrost-platform,bifrost-ui".',
    '   Poll every 10s (up to 2 min) until both repos show the pushed commits on **main**. This confirms Gitea mirror sync.',
    '',
    '### Phase B — Deploy STG',
    '8. Call get_release_state to confirm next_action is deploy_stg.',
    '9. Call start_pipeline_run with pipeline="bifrost-deliver-platform" and revision="main" (unless operator specified another common ref).',
    '10. Poll get_pipeline_runs for "bifrost-deliver-platform" every 15s until the run reaches "succeeded" or "failed".',
    '11. If failed: analyze the run status, report the error, and stop.',
    '',
    '### Phase C — Gate STG',
    '12. Call request_operator_approval to confirm: "STG deploy succeeded. Run STG gate?"',
    '13. On approval, call run_release_gate with tier="platform-stg".',
    '14. Report the gate result (pass/fail, checks, blockers).',
    '15. If failed: report blockers and stop.',
    '',
    '### Phase D — Deploy PROD',
    '16. Call request_operator_approval: "STG gate passed. Deploy the same revision to PROD?"',
    '17. On approval, call start_pipeline_run with pipeline="bifrost-deliver-platform-prod" and the SAME revision as STG (usually "main").',
    '18. Poll get_pipeline_runs for "bifrost-deliver-platform-prod" until succeeded/failed.',
    '',
    '### Phase E — Gate PROD',
    '19. Call request_operator_approval: "PROD deploy succeeded. Run PROD gate?"',
    '20. On approval, call run_release_gate with tier="platform-prod".',
    '21. Report the gate result.',
    '',
    '### Phase F — Verify & Report',
    '22. Call get_release_state and confirm all four stages show pass/succeeded.',
    '23. **Console theme smoke (mandatory when console or bifrost-ui changed):**',
    ...
    '24. Generate a Release Report summarizing:',
