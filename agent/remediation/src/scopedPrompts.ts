...
export function buildMassiveFeedRecoverRunnerPrompt(req: StartRunRequest): string {
  return [
    ...
    '1. verify_mission_snapshot + verify_payload — classify PROBE_DRIFT vs DATA_LAYER vs HTTP_FAIL for market-data / polygon targets.',
    '2. get_cluster_summary — look for polygon-ws-ingestor / market-data-api / redis-massive CrashLoop or ImagePull (plugin-market-data NS).',
    '3. If pod unhealthy: rollout_restart_deployment (operator approval) for the failing Deployment only.',
    '4. If API key / config: request_operator_manual_steps to rotate Polygon credentials in Market Data Plugin config (do not invent secrets).',
    '5. Re-run verify_mission_snapshot; confirm massive-polygon / market-data reachability=ok before closing.',
    '',
    '## Must-not',
    `- ${D10_MUST_NOT}`,
