...
export function buildPluginRuntimeRemediateRunnerPrompt(req: StartRunRequest): string {
  return [
    ...
    '',
    '## Workflow (strict)',
    '1. Detect — read operator JSON context first. Optional: get_ib_gateway_plugin_status / get_market_data_plugin_status / market_data_doctor if available.',
...
export function buildMassiveFeedRecoverRunnerPrompt(req: StartRunRequest): string {
  return [
    ...
    '1. market_data_doctor — the plugin names what the session is missing (snapshot / OI / bars / ratios), stale slots, failed jobs, worker and vendor health; every finding carries a prescription.',
    '2. If the doctor lists prescriptions: market_data_heal dry_run=true to preview, request_operator_approval, then market_data_heal (optionally finding_ids). This enqueues the exact slot for the exact session; nothing else.',
    '3. Wait for the queue to drain (get_market_data_plugin_status / queue-dashboard), then market_data_doctor again. Healthy → close. Still critical → continue.',
    '4. Findings the plugin cannot fix itself: worker unreachable → rollout_restart_deployment (operator approval) for that Deployment only; vendor 401/403 → request_operator_manual_steps to rotate Polygon credentials (do not invent secrets); verify_mission_snapshot + verify_payload for PROBE_DRIFT vs DATA_LAYER vs HTTP_FAIL.',
    '5. Re-run market_data_doctor and verify_mission_snapshot; confirm verdict healthy and market-data reachability=ok before closing.',
    '',
    '## Must-not',
    `- ${D10_MUST_NOT}`,
